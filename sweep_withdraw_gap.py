...
import numpy as np
import math
from hypostases_ref import Characteristics, WorldModel, GoalHierarchy, PowerExternal, AgentState
from hypostases_infer import infer, summarize_kalman, reseed_infer

def make_test_agent(name, reserve0):
    """D_low/D_high construction from §12.5, factored out for reuse."""
    c = Characteristics(skill=0.6, resilience=0.5, sociality=0.5,
                         memory_decay=0.9, reserve=reserve0, mood=0.0)
...
def generate_forced_withdraw_trace(agent, pool0, n_steps):
    from hypostases_ref import step_env, feedback, evolve, Action, ActionType
    pool = pool0
...
def single_trial(n_steps, n_particles, seed, reserve_low=3.0, reserve_high=14.0):
    ...
    particles_high = infer(actions_high, pools_high, xi, n_particles=n_particles, agent_name="D_high")
    ...
    z = (k_high["reserve_mean"] - k_low["reserve_mean"]) / pooled_sd if pooled_sd > 0 else float("nan")

    return {
        "z": z,
        "mu_low": k_low["reserve_mean"], "var_low": k_low["reserve_var"],
        "mu_high": k_high["reserve_mean"], "var_high": k_high["reserve_var"],
    }
...
def evaluate_config(n_steps, n_particles, seeds):
    ...
    cond1 = cond1_count >= math.ceil(0.8 * S)
    ...
    degenerate_count = sum(
        1 for r in results
        if not (0.5 <= r["var_low"] <= 20) or not (0.5 <= r["var_high"] <= 20)
    )
    ...
    return {
        ...
        "passed": passed,
        "cond1_directional": cond1, "cond1_count": cond1_count, "S": S,
        "cond2_median_z": median_z, "cond2_pass": cond2,
        "cond3_degenerate_count": degenerate_count, "cond3_pass": cond3,
        "all_z": zs,
    }
...
if __name__ == "__main__":
    ...
    for n_steps, n_particles in configs:
        ...
        print(f"{n_steps:>8} {n_particles:>12} {r['cond2_median_z']:>10.3f} "
              f"{r['cond1_count']}/{r['S']:>6} {str(r['passed']):>8}")
    ...
    for coupling in [0.08, 0.3, 0.6, 1.0, 2.0, 4.0]:
        ...
        print(f"{coupling:>10.2f} {r['cond2_median_z']:>10.3f} "
              f"{r['cond1_count']}/{r['S']:>6} {str(r['passed']):>8} {r['cond3_degenerate_count']:>6}")
